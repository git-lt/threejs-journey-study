...
const simpleShadow = textureLoader.load('/public/assets/textures/simpleShadow.jpg')
...
const shawdowPlane = new THREE.Mesh(
...
shawdowPlane.rotateX(-Math.PI / 2);
shawdowPlane.position.y = plane.position.y + .01;

scene.add(shpere,  plane, shawdowPlane)
...
const tick = () => {
  ...
  shpere.position.x = Math.sin(elapsedTime) * 1.5;
  shpere.position.z = Math.cos(elapsedTime) * 1.5;
  shpere.position.y = Math.abs(Math.sin(elapsedTime * 2.5))

  shawdowPlane.position.x = shpere.position.x;
  shawdowPlane.position.z= shpere.position.z;
  shawdowPlane.material.opacity = (1 - shpere.position.y) * .6;
